...
import serial
import time   # ✅ ADDED (needed for home delays)
...
# ============================================================================
# ✅ ADDED: HOME POSITION SUPPORT (Home → Move → Home)
# ============================================================================
HOME_PULSES   = [1500, 1500, 1500, 1500]  # Servo 0..3 (change after calibration)
HOME_TIME_MS  = 2000

def go_home():
    """Move arm to HOME pulses."""
    cmd = (
        f"#0 P{HOME_PULSES[0]} "
        f"#1 P{HOME_PULSES[1]} "
        f"#2 P{HOME_PULSES[2]} "
...
def calculate_angles_from_coordinates(x_cm, y_cm, z_cm):
    ...
    # ✅ ADDED: Always start at HOME before each move
    go_home()
    ...
    # ===== SEND TO ARM =====
    send_pwm_command(cmd)
    time.sleep(2.3)  # ✅ ADDED: wait for move to complete

    # ✅ ADDED: return HOME after reaching target
    go_home()
